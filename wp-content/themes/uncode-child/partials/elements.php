	...
	function uncode_create_single_block($block_data, $el_id, $style_preset, $layout, $lightbox_classes, $carousel_textual, $with_html = true, $is_default_product_content = false)
	{
		...
		if (array_key_exists('media',$layout) || $single_text === 'overlay') :
			...
			if (($single_text !== 'overlay' || $single_elements_click !== 'yes') && $media_type === 'image'):
				...
				// Custom changes by Phases.
                global $post;
               if($post->post_type == 'asx-announcement')
               {
                   $lightbox_data.$data_values = str_replace('target="_self"','target="_blank"',$lightbox_data.$data_values);
               }
			...
			if (array_key_exists('media',$layout)) :
				...
				if ($style_preset === 'metro'):
					...
					if ( isset( $secondary_featured_image ) && $secondary_featured_image !== false ) {
						...
						$secondary_bg_cover = '<div class="t-secondary-background-cover'.($adaptive_async_class !== '' ? $adaptive_async_class : '').'" style="background-image:url(\''.$item_media.'\')"'.($secondary_async_data !== '' ? $secondary_async_data : '').'></div>';
					...
					if ($media_type === 'image') :
						...
						if ( isset( $media_attributes->id ) ) {
							$adaptive_async_class .= ' wp-image-' . $media_attributes->id;
						}

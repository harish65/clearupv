...
if ($layout === 'sidebar_right' || $layout === 'sidebar_left') {
    ...
    if ($generic_body_content_block === '') {
        $the_content = '<div class="post-content1 un-no-sidebar-layout"' . $page_custom_width . '>' . uncode_get_row_template($the_content, $limit_width, $limit_content_width, $style, '', 'double', true, 'double') . '</div>';
    } else {
...
?>
<!-- November 23 -->
<div class="page-body <?php echo $bg_color; ?>">
    <div class="post-wrapper">
        <div class="post-body">
            <div class="post-content un-no-sidebar-layout">
                <div class="row-container">
                    <section class="section board-directors tab-wrap">
                        <div class="container">
                            <div class="tab">
                                <ul class="tabs-nav limit-width single-h-padding">
                                    <?php
                                    $team_category = get_terms(
                                        array(
                                            'taxonomy' => 'team_member_category',
                                            'hide_empty' => true,
                                        )
                                    );
                                    if (!empty($team_category) && is_array($team_category)) {
                                        $i = 1;
                                        foreach ($team_category as $term) {
                                            if ($i == '1') {
                                                $first_term = $term->term_id;
                                            } ?>
                                            <li>
                                                <a href="#member-<?php echo $term->term_id; ?>">
                                                    <?php echo $term->name; ?>
                                                </a>
                                            </li>
                                            <?php
                                ...
                                <div class="tabs-content"> <!-- tabs-content -->
                                    <?php
                                    $team_category1 = get_terms(
                                        array(
                                            'taxonomy' => 'team_member_category',
                                            'hide_empty' => true,
                                        )
                                    );
                                    if (!empty($team_category1) && is_array($team_category1)) {
                                        $i = 1;
                                        foreach ($team_category1 as $term1) {
                                            $team_category_title1 = get_field('team_category_title', $term1);
                                            $team_category_description1 = get_field('team_category_description', $term1);
                                            $args1 = array(
                                            ...
                                            if ($query1->have_posts()) {
                                                ?>
                                                <div class="tab-content" id="member-<?php echo $term1->term_id; ?>">
                                                    <div class="cold-lg-12 col-md-12 limit-width single-h-padding">
                                                        <?php
                                                        if (!empty($team_category_title1)) {
                                                            echo '<h3>' . $team_category_title1 . '</h3>';
                                                        }
                                                        if (!empty($team_category_description1)) {
                                                            echo $team_category_description1;
                                                        }
                                                        ?>
                                                    </div>
                                                    <div class="limit-width single-h-padding">
                                                        <div class="row profile_box-wrapper">
                                                            <?php
                                                            while ($query1->have_posts()) {
                                                                $query1->the_post();
                                                                if (have_rows('team_member_details')) {
                                                                    while (have_rows('team_member_details')) {
                                                                        the_row();
                                                                        $member_ID = get_the_ID();
                                                                        $member_title = get_the_title();
                                                                        $designation = get_sub_field('team_member_designation');
                                                                        $description = get_sub_field('team_member_description');
                                                                        $profile = get_sub_field('team_member_profile_image');
                                                                        $profile_short_image = wp_get_attachment_image_url($profile['ID'], 'team-short-thumbnail');
                                                                        ?>
                                                                        <div class="col-lg-3 col-md-3 col-sm-4  col-6">
                                                                            <a href="#tm<?php echo $member_ID; ?>-<?php echo $term1->term_id;?>"
                                                                               class="profile-box tm<?php echo $member_ID; ?>-<?php echo $term1->term_id;?>">
                                                                                <?php
                                                                                if (!empty($profile_short_image)) {
                                                                                    ?>
                                                                                    <div class="img-wrapper">
                                                                                        <img src="<?php echo $profile_short_image; ?>"
                                                                                             alt="<?php echo $member_title; ?>">
                                                                                    </div>
                                                                                    <?php
                                                                                }
                                                                                ?>
                                                    ...
                                                    <div class="row profile-contemt-wrapper">
                                                        <?php
                                                        while ($query1->have_posts()) {
                                                            $query1->the_post();
                                                            if (have_rows('team_member_details')) {
                                                                while (have_rows('team_member_details')) {
                                                                    ...
                                                                    $designation = get_sub_field('team_member_designation');
                                                                    $profile = get_sub_field('team_member_profile_image');
                                                                    $profile_image = wp_get_attachment_image_url($profile['ID'], 'team-full-thumbnail');
                                                                    $description_title = get_sub_field('team_member_description_title');
                                                                    $description = get_sub_field('team_member_description');
                                                                    $email_address = get_sub_field('team_member_email_address');
                                                                    $phone_number = get_sub_field('team_member_phone_number');
                                                                    $linked_in_url = get_sub_field('team_member_linked_in_url');
                                                                    ?>
                                                                    <div class="col-md-12 profile-detailed-view_wrapper limit-width single-h-padding"
                                                                         id="tm<?php echo $member_ID; ?>-<?php echo $term1->term_id;?>">
                                                                        <div class="profile-detailed-view row">
                                                                            ...
                                                                            <div class="col-md-8">
                                                                                <div class="profile-detailed-content">
                                                                                    <h3>
                                                                                        <?php echo $member_title; ?>
                                                                                    </h3>
                                                                                    <?php
                                                                                    if (!empty($designation)) {
                                                                                        echo '<h4>' . $designation . '</h4>';
                                                                                    }
                                                                                    if (!empty($description_title)) {
                                                                                        echo '<h5>' . $description_title . '</h5>';
                                                                                    }
                                                                                ...
                                                                                <?php
                                                                                if (!empty($email_address) || !empty($phone_number) || !empty($linked_in_url)) { ?>
                                                                                    <div class="content-bottom social-media-links">
                                                                                        <?php
                                                                                        if (!empty($email_address)) {
                                                                                            echo '<a class="email-id" href="mailto:' . $email_address . '">' . $email_address . '</a>';
                                                                                        }
                                                                                        if (!empty($phone_number)) {
                                                                                            echo '<a class="phone-number"  href="tel:'.str_replace(" ", "",$phone_number). '">' . $phone_number . '</a>';
                                                                                        }
                                                                                        if (!empty($linked_in_url)) {
                                                                                            ?>
                                                                                            <div class="scial-icon">

                                                                                                <a class="linked-in"
                                                                                                   href="<?php echo $linked_in_url; ?>"
                                                                                                   target="_blank">
                                                                                                    <?php echo $linked_in_url; ?>
                                                                                                </a>
                                                                                            </div>
                                                                                            <?php
                                                                                        }

...
function uncode_register_required_plugins() {
	...
	if ( ! apply_filters( 'uncode_enable_remote_uncode_core', false ) ) {
		$uncode_core_conf = uncode_get_uncode_core_plugin_conf();

		$plugins[] = array(
			'name'               => $uncode_core_conf['plugin_name'], // The plugin name.
			'slug'               => $uncode_core_conf['plugin_slug'], // The plugin slug (typically the folder name).
			'source'             => get_template_directory() . '/core/plugins_activation/plugins/uncode-core.zip', // The plugin source.
			'required'           => true, // If false, the plugin is only 'recommended' instead of required.
			'version'            => $uncode_core_conf['version'], // E.g. 1.0.0. If set, the active plugin must be this version or higher. If the plugin version is higher than the plugin version installed, the user will be notified to update the plugin.

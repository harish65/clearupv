...
$attributes_first = array(
	...
	'sticky_single_image_position' => '',
	'single_vertical_text' => '',
	'sticky_single_vertical_text' => '',
	'single_image_size' => '6',
	'sticky_single_image_size' => '6',
	'single_lateral_responsive' => 'yes',
	'sticky_single_lateral_responsive' => 'yes',
	'drop_image_position' => '',
	'drop_image_time' => '25',
	'drop_image_default' => '',
	'drop_title_default' => '',
	...
	'single_secondary' => '',
	'single_reduced' => '',
	'sticky_single_reduced' => '',
	'single_reduced_mobile' => '',
	'sticky_single_reduced_mobile' => '',
	'single_padding' => '',
	'single_padding_vertical' => '',
	'sticky_single_padding_vertical' => '',
	'single_text_reduced' => '',
	'single_h_align' => 'left',
	'single_h_align_mobile' => '',
	'single_table_last_align' => '',
	'single_v_position' => 'middle',
	'sticky_single_v_position' => 'middle',
	'single_h_position' => 'left',
	'sticky_single_h_position' => '',
	'single_style' => 'light',
	...
	'navigation_prev_label' => '',
	'navigation_prev_icon' => '',
	'navigation_next_label' => '',
	'navigation_next_icon' => '',
	'navigation_label_custom_typo' => '',
	'navigation_label_font' => '',
	'navigation_label_size' => '',
	'navigation_label_weight' => '',
	'navigation_label_transform' => '',
...
$general_width = $index_type === 'sticky-scroll' ? 12/$sticky_th_grid_lg : $single_width;
...
// Populate navigation posts when needed
if ( $auto_query === 'yes' && $auto_query_type === 'navigation' ) {
	$needed_nav_posts = $navigation_type === 'next' || $navigation_type === 'prev' ? 1 : 2;

	if ( $needed_nav_posts > count( $posts ) ) {
		if ( get_post_type( $this->current_post_id ) !== 'uncodeblock' ) {
			while ( $needed_nav_posts > count( $posts ) ) {
				$post = new stdClass(); // Creating post object.
				$post->id = $this->current_post_id;
				$post->title = get_the_title($post->id);
				$post->type = get_post_type( $post->id );
				$post->format = ($post->type === 'post') ? get_post_format( $post->id ) : '';
				$post->link = get_permalink( $post->id );
...
if ($index_type == 'isotope') {
	...
	if ( ! $is_tax_query ) {
		if ( $infinite === 'yes' && $pagination !== 'yes')  {
			$container_classes[] = 'cssgrid-infinite grid-infinite';
			if ($infinite_button === 'yes') {
				$container_classes[] = 'cssgrid-infinite-button grid-infinite-button';
			}
		} else {
			$container_classes[] = 'cssgrid-pagination grid-pagination';
		}
	}
	if ($index_back_color !== '') {
		$parent_container_classes[] = 'style-'.$index_back_color.'-bg';
...
} elseif ($index_type === 'sticky-scroll') {
	...
	if ( ! $is_tax_query ) {
		if ( $infinite === 'yes' && $pagination !== 'yes')  {
			$parent_container_classes[] = 'index-infinite grid-infinite';
			if ($infinite_button === 'yes') {
				$parent_container_classes[] = 'index-infinite-button grid-infinite-button';
			}
		} else {
			$parent_container_classes[] = 'index-pagination grid-pagination';
		}
	}
	if ($index_back_color !== '') {
		$parent_container_classes[] = 'style-'.$index_back_color.'-bg';
						...
						<div class="menu-horizontal<?php echo esc_attr( ($filtering_full_width !== 'yes') ? ' limit-width' : '' ); ?><?php echo esc_attr( $menu_sub_enhanced ); ?> text-<?php echo esc_attr($filtering_position); ?> text-mobile-<?php echo esc_attr($filter_mobile_align); ?> filter-alignment-<?php echo esc_attr($filtering_position); ?> <?php echo esc_attr( ( $woo_sorting_shadow ) ? ' menu-dd-shadow-sm' : '' ); ?>">
							<?php if ( ! $filter_hide_cats ) : ?>
								...
								<ul class="menu-smart sm<?php echo esc_attr( ($filtering_uppercase === 'yes') ? ' text-uppercase' : ' no-text-uppercase' ); ?> menu-smart--filter-cats <?php echo esc_attr( $filter_mobile_dropdown === 'yes' ? 'menu-smart--filter-cats-mobile-dropdown ul-mobile-dropdown' : '' ); ?>">
									...
									?>
									<?php
									$isotope_nav_link_class = $show_woo_sorting || $show_widgetized_content_block ? '' : 'isotope-nav-link grid-nav-link';
									?>
							...
							<?php endif; ?>
								...
								<?php if ( $show_widgetized_content_block ) : ?>
									...
									<ul class="menu-smart sm uncode-woocommerce-toggle-widgetized-cb">
										<li class="uncode-woocommerce-toggle-widgetized-cb__item"><a href="#" class="uncode-woocommerce-toggle-widgetized-cb__link no-isotope-filter no-grid-filter mobile-toggle-trigger"><?php echo esc_html( $widgetized_content_block_toggle_text ); ?></a>
										</li>
									</ul>
								<?php endif; ?>
		...
		<?php if ( !empty( $parent_container_classes ) ) { ?><div class="<?php echo esc_attr(trim(implode(' ', $parent_container_classes))); ?>"<?php echo uncode_switch_stock_string( $max_w_ajax_grid_style ); ?>><?php } ?>
			...
			if ( $posts_counter > 0 ):
				...
				foreach ( $posts as $key_post => $post ):
					...
					$block_data['single_width'] = $single_height_viewport === 'yes' || $thumb_size === 'fluid' || $sticky_thumb_size === 'relative' ? '12' : $single_width;
					...
					if ( $auto_query_type === 'navigation' ) {
						$block_data['is_navigation']             = true;
						if ( $navigation_type === 'next' ) {
							$block_data['navigation_index'] = 'next';
						} else if ( $navigation_type === 'prev' ) {
							$block_data['navigation_index'] = 'prev';
						} else {
							$block_data['navigation_index'] = $i_matrix === 1 ? 'next' : 'prev';
						}

						$block_data['is_navigation']                = true;
						$block_data['navigation_type']              = $navigation_type;
						$block_data['navigation_label_position']    = $navigation_label_position;
						$block_data['navigation_icon_position']     = $navigation_icon_position;
						$block_data['navigation_prev_label']        = $navigation_prev_label;
						$block_data['navigation_prev_icon']         = $navigation_prev_icon;
						$block_data['navigation_next_label']        = $navigation_next_label;
						$block_data['navigation_next_icon']         = $navigation_next_icon;
						$block_data['navigation_label_custom_typo'] = $navigation_label_custom_typo;
						$block_data['navigation_label_font']        = $navigation_label_font;
						$block_data['navigation_label_size']        = $navigation_label_size;
						$block_data['navigation_label_weight']      = $navigation_label_weight;
						$block_data['navigation_label_transform']   = $navigation_label_transform;
						$block_data['navigation_label_height']      = $navigation_label_height;
						$block_data['navigation_label_space']       = $navigation_label_space;
					} else {
						$block_data['navigation'] = false;
					}

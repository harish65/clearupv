...
class Vc_Templates_Panel_Editor {
		...
		if ( 'my_templates' === $category['category'] ) {
			...
			if ( 'default_templates' === $category['category'] ) {
				...
				<div class="vc_column vc_col-sm-12">
					...
					// BEGIN UNCODE EDIT
					$category['output'] .= apply_filters( 'uncode_vc_before_templates_list', $category['output'] );
					// END UNCODE EDIT
				if ( ! empty( $category['templates'] ) ) {
					foreach ( $category['templates'] as $template ) {
						// BEGIN UNCODE EDIT
						$category['output'] .= apply_filters( 'uncode_vc_before_single_template', '', $template );
						// END UNCODE EDIT

						$category['output'] .= $this->renderTemplateListItem( $template );

						// BEGIN UNCODE EDIT
						$category['output'] .= apply_filters( 'uncode_vc_after_single_template', '' );
						// END UNCODE EDIT
					}
				}
					// BEGIN UNCODE EDIT
					$category['output'] .= apply_filters( 'uncode_vc_after_templates_list', '' );
					// END UNCODE EDIT
				$category['output'] .= '
				</div>
			</div>';
	...
	public function renderFrontendDefaultTemplate() {
		// BEGIN UNCODE EDIT
		do_action( 'uncode_render_frontend_template' );
		// END UNCODE EDIT
	...
	public function getAllTemplates() {
		...
		if ( apply_filters( 'vc_show_user_templates', true ) ) {
			...
			$arr_category = array(
				...
				// BEGIN UNCODE EDIT
				'category_name' => apply_filters( 'uncode_template_tab_title', esc_html__( 'My Templates', 'js_composer' ) ),
				// END UNCODE EDIT
			...
			if ( ! empty( $user_templates ) ) {
				foreach ( $user_templates as $template_id => $template_data ) {
					// BEGIN UNCODE EDIT
					$category_templates[] = apply_filters( 'uncode_vc_category_templates_args', array(
						'unique_id' => $template_id,
		...
		if ( ! empty( $default_templates ) ) {
			// BEGIN UNCODE EDIT
			$arr_category = apply_filters( 'uncode_vc_category_templates_name_args', array(
			...
			foreach ( $default_templates as $template_id => $template_data ) {
				...
				// BEGIN UNCODE EDIT
				$category_templates[] = apply_filters( 'uncode_vc_default_category_templates_args', array(
					'unique_id' => $template_id,
					'name' => $template_data['name'],
	...
	public function getBackendDefaultTemplate( $return = false ) {
		// BEGIN UNCODE EDIT
		do_action( 'uncode_render_backend_template' );
		// END UNCODE EDIT
	...
	public function getAllTemplatesSorted() {
		...
		foreach ( $data as $key => $category ) {
			...
			$data[ $key ]['templates'] = $category['templates'];
			// END UNCODE EDIT
		}
...
HTML;
		// BEGIN UNCODE EDIT
		$output .= apply_filters( 'uncode_vc_templates_html', '', $name, $template, $template_type );
		// END UNCODE EDIT
